...
if(!localStorage.getItem("surveyNames")){
    localStorage.setItem("surveyNames", "def ");
    localStorage.setItem("def", "<p>this is htmlcode</p>")
}
function openTest(){
    document.getElementById("result").innerHTML = localStorage.getItem("def");
}
    // TRY TWO
function openSaved() {
    document.getElementById("result").innerHTML = "Stored:" + localStorage.getItem("temporary");

}

function handleText(){
    document.getElementById("result").innerHTML = "<p>Sorted: </p>"
    var outAry = (localStorage.getItem("surveyNames")).split(",");
    for (var i in outAry) {
        document.getElementById("result").innerHTML = "<button class = 'buts' onclick='openSaved()'>" + outAry[i] + "</button>";
    }
}
...
function myfun(){
    var t = $("#n").val() + "" ;
    localStorage.setItem("names", localStorage.getItem("names") + " " + t);
}
function update(){
    document.getElementById("result").innerHTML = "Stored: " + localStorage.getItem("names") ;
}
*/
$(document).ready(function(){
    $(".optionButton").click(function(){
        survey2 = $("#survey").html();
        localStorage.setItem("temporary",""+ survey2);
        //alert("HTML: " + survey2);
    });
});
...
/** delete an option that was addded
 ...
 */
function newSurvey(){
    var temp = $("#surName").val() + "";
    localStorage.setItem("surveyNames", localStorage.getItem("surveyNames") + " " + temp);
}
...
function addToSurvey(panName, divName) {
    ...
    switch (panName) {
        case "surveyForm":
            var temp = localStorage.getItem("surveyNames") + "," + ($("#surName").val()) ;
            if(!localStorage.getItem("" + ($("#surName").val()))){
                localStorage.setItem("" + ($("#surName").val()),  $("#survey").html());
                localStorage.setItem("surveyNames", temp);
            }
            break;
        case "stForm":
            $('#survey').append('<div class="question" id="q' + questionNumber + '"></div>');
            $('#q' + questionNumber).append('<button style="position:absolute;right:5%;border:none"' +
